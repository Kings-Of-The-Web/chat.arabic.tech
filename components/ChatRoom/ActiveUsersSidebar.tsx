  ...
  return (
    <Card className="h-full bg-white dark:bg-gray-800">
      <div className="p-4">
        <div className="mb-4 flex items-center gap-2">
          <Users className="h-5 w-5 text-violet-500 dark:text-violet-400" />
          <h2 className="font-hacen font-bold text-gray-900 dark:text-white">المستخدمون النشطون</h2>
        </div>
        <ScrollArea className="h-[calc(100vh-12rem)]">
          <div className="space-y-2 pl-1">
            {users.map((user) => (
              ...
              >
                <span className="font-hacen text-gray-900 dark:text-white">{user.name}</span>
                <span
                  className={`h-2.5 w-2.5 rounded-full ${
                    user.isOnline ? 'animate-pulse bg-green-500' : 'bg-gray-300 dark:bg-gray-600'
                  }`}
                />
              </div>
            ))}
